...
- **Content Series**: A collection of related content items (also called a content collection). Key attributes include: unique identifier, title, description, creation date, last modified date, created by (user identifier), total content count, status (active/soft-deleted). Relationships: Contains multiple content items (cascade delete); can have content series-specific editor permissions
- **Content Item**: Individual piece of content within a content series. Key attributes include: unique identifier, title, body text, author (user identifier), content series identifier, current version number, status (draft/published/archived), creation date, publication date, last modified date, view count. Relationships: Belongs to one content series; created by one user; has multiple versions
...
- **Content Series Permission**: Links users to content series for granular editing permissions (referenced from admin-user spec). Key attributes include: user identifier, content series identifier, permission level (Editor), granted by (admin identifier). Relationships: Enables Members to edit specific content series; deleted when content series is deleted
